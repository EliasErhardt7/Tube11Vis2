...
void InitD3D(HWND hWnd)
{
    ...
    
    {
        ...
        hr = D3DCompileFromFile(L"shaders/phong.hlsl", 0, 0, "PSMain", "ps_4_0", 0, 0, &modelShader.psBlob, &errorBlob);
        if (FAILED(hr))
        {
            if (errorBlob)
            {
    ...
    {
        auto hr = D3DCompileFromFile(L"shaders/quadcomposite.hlsl", 0, 0, "VSMain", "vs_4_0", 0, 0, &quadCompositeShader.vsBlob, &errorBlob);
        if (FAILED(hr))
        {
            if (errorBlob)
            {
    ...
    // compute shaders
    {
        auto hr = D3DCompileFromFile(L"shaders/thresholddownsample.hlsl", 0, 0, "ThresholdAndDownsample", "cs_5_0", 0, 0, &thresholdDownsampleShader.csBlob, &errorBlob);
        if (FAILED(hr))
        {
            if (errorBlob)
            {
    ...
    {
        auto hr = D3DCompileFromFile(L"shaders/blur.hlsl", 0, 0, "Blur", "cs_5_0", 0, 0, &blurShader.csBlob, &errorBlob);
        if (FAILED(hr))
        {
            if (errorBlob)
            {
